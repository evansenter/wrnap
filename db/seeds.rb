...
Parallel.each(entries, in_processes: 0, progress: "Populating SQLite DB") do |stockholm|
  family = Wrnap::DB::RfamFamily.find_or_create_by(
    name:                stockholm.gf_features["AC"],
    description:         stockholm.gf_features["DE"],
    consensus_structure: stockholm.gc_features["SS_cons"]
  )

  existing_sequences = family.seqs.map { |sequence| sequence.instance_eval { "%s/%d-%d" % [accession, from, to] } }.to_set
  sequences          = stockholm.records.reject do |key, _|
    ...
    Wrnap::DB::RfamSequence.find_or_initialize_by(
      rfam_family: family,
      accession:   accession,
      from:        from,
      to:          to
  ...
  Wrnap::DB::RfamSequence.import(sequences.select(&:new_record?))
end and nil
