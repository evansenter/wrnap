module Wrnap
  module RnaInitializer
    ...
    alias_method :init_from_fa, def init_from_fasta(string)
      if !File.exists?(string) && string !~ /\.fa$/ && File.exists?(string + ".fa")
        string += ".fa"
      end
      ...
      if File.exists?(string)
        ...
        if string.count(?>) > 1
          # More than one sequence in the Fasta file
          ...
          # Just one sequence in the Fasta file
        ...
        raise ArgumentError.new("No such file exists")
      end
    end
